...
describe("ProjectPage", () => {
  ...
  it("hosts the project chat, wired to the same open flow as the list", async () => {
    const onSend = vi.fn();
    const user = userEvent.setup();
    renderPage({ chat: { ...idleChat, onSend } });

    const chat = screen.getByRole("region", { name: /project chat/i });
    await user.type(within(chat).getByRole("textbox", { name: /ask about/i }), "когда дедлайн?");
