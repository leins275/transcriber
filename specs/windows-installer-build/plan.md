...
### [ ] T1: Root Makefile QA fanout + frozen-lock verification  [deps: —]
...
### [ ] T2: Developer bootstrap script  [deps: —]
...
### [ ] T3: Product version single source of truth  [deps: —]
...
### [ ] T4: Bake a relocatable Python runtime + dependency tree  [deps: —]
...
### [ ] T5: Service payload dependencies — CUDA runtime wheels, huggingface_hub, DLL shim  [deps: T3]
...
### [ ] T6: Tauri bundle configuration — resources, NSIS per-user install, WebView2, CRT  [deps: T3, T4]
...
### [ ] T7: NSIS installer hooks — app folder skeleton, upgrade preservation, uninstall policy, silent mode  [deps: —]
...
### [ ] T8: One-command release build — orchestration, checksum, manifest, size gate  [deps: T3, T4]
...
### [ ] T9: App-folder resolution and sidecar launch against the bundled runtime  [deps: T5, T6]
...
### [ ] T10: Model download core — resume, verify, cancel, progress  [deps: T5]
...
### [ ] T11: Expose the downloader over HTTP and the CLI  [deps: T10]
...
### [ ] T12: Build, install and configuration documentation + manual smoke checklist  [deps: T1, T2, T7, T8]
...
### [ ] T13: First-run model-download step and missing-model recovery in the app  [deps: T9, T11]
...
### [ ] T14: End-to-end installer verification on the operator's machine  [deps: T6, T7, T8, T9, T12, T13]
