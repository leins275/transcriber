...
### [ ] T1: Python — delete the `report` job type end-to-end  [deps: —]
...
### [ ] T2: Rust (Tauri app) — delete report + artifact-browsing commands  [deps: —]
...
### [ ] T3: Frontend — rebuild ProjectPage as a recordings-only page  [deps: —]
...
### [ ] T4: vault crate — remove report listing (and artifact listing if F6 left it uncalled)  [deps: T2]
...
### [ ] T5: Frontend — purge `"report"` and dead artifact types from api/types/JobRow + copy  [deps: T3]
...
### [ ] T6: Integration — full QA gates, acceptance greps, live app smoke  [deps: T1, T2, T3, T4, T5]
