...
def _meeting_tree(tmp_path: Path) -> tuple[Path, Path]:
    """``(meeting_dir, export_dir)`` in the layout ``build_export_md``
    consumes via ``items_for_meeting`` -- items live inside the meeting
    folder, not at the project level."""
    ...
    return meeting_dir, export_dir


def _write_action_item(meeting_dir: Path, *, archived: bool) -> Path:
    return write_item(
        meeting_dir / ACTION_ITEMS_DIR_NAME,
        title="Fix login",
        meta={
            "type": "task",
            "title": "Fix login",
...
def test_export_output_is_identical_whether_an_item_is_archived(tmp_path: Path) -> None:
    meeting_dir, export_dir = _meeting_tree(tmp_path)
    md_path = _write_action_item(meeting_dir, archived=False)
...
def test_list_items_includes_an_archived_item_exactly_like_an_unarchived_one(
    ...
    meeting_dir, _export_dir = _meeting_tree(tmp_path)
    md_path = _write_action_item(meeting_dir, archived=False)
    items_dir = meeting_dir / ACTION_ITEMS_DIR_NAME
