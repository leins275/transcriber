...
make lint     # clippy -D warnings | eslint | ruff check | sync_version --check | verify_locks --check | gen_diarization_runtime --check
...
**Hybrid search + chat + MCP.** The service owns a rebuildable search index (`<vault_root>/.transcriber/index.sqlite3` — it travels with its vault; app-dir fallback only when no vault root is set; sqlite-vec + FTS5, bge-m3 GGUF embeddings on CPU) filled by the `index` job, which the app fires quietly after every finished job and note save. Settings offers the bge-m3 download ("Enable vector search", the `embedding_model_download` command trio); docs indexed text-only re-embed automatically on the first pass after the model lands. `POST /v1/search` fuses vector/BM25/exact-title/trigram channels with weighted RRF; `POST /v1/chat` streams a RAG answer (SSE) that the Rust shell forwards to React over a `tauri::ipc::Channel`. `transcriber-mcp` (a console script in the Python package) is a standalone stdio MCP server over the same vault+index for Claude Desktop — works with the app closed. Per-meeting `note.md` (edited in the app) sits on top. The chat lives as the library's third tab (redesign turn 9 — there are no project pages): project selector in the tab, an index-status chip/panel backed by `GET /v1/index/status`, and conversations persisted as JSON in the vault's reserved `<PROJECT>/chats/` directory (list/read/save/rename/delete via `commands/chats.rs`; sources stored by vault-relative dir, resolved to entry ids on read). `speakers.json` names feed project-level suggestions (in open mode — see the project roster below), and diarization stores per-speaker voice embeddings in `transcript.json`; after a diarized transcription, cross-meeting speaker recognition (`speaker_matching.py`, threshold `speaker_match_threshold`) pre-names voices already named in sibling meetings — additive only, operator assignments always win. Decode languages: ru/en/tr (constrained auto-detect).

**Speaker identification: the runtime is a first-run payload, the models are baked.** The installer never ships pyannote/torch; Settings' Speakers row fetches (1) the runtime — every wheel the `diarization` extra adds on top of the baked env, with torch/torchaudio swapped for their `cu126` CUDA builds (~3 GB; torch 2.8, the last series with the torchaudio API pyannote 3 needs), pinned in `diarization_runtime_packages.py`, which `scripts/gen_diarization_runtime.py` generates from `uv.lock` (`make lint` checks the drift) — into `<app_dir>/runtime/diarization/` (put on `sys.path` by `diarizer.py`), while (2) the three pinned pyannote snapshots (~32 MB) are **committed** under `apps/desktop/src-tauri/resources/models/diarization/` and bundled to `<install dir>\models\diarization\` — no build or release needs a Hugging Face token; `build_installer.py`'s `diarization_models_check` stage fails the build if the committed tree drifts from the revisions pinned in `diarization_runtime.py` (re-pin = change the pins, refresh once with `download-diarization-models --out` and a token, commit both). The in-app token box (`hf_token`, typed in `config.rs` alongside `diarize`, both written by `set_diarization_settings`, which restarts the sidecar) only appears on a build without them. Once fetched, the pipeline loads offline (`refs/main` pinned, hub forced offline for the load). NVIDIA GPU only. The project's voice memory stays the on-demand sibling scan — there is deliberately no `<PROJECT>/speakers.json`; the `diarize` job (`commands/speakers.rs`, "Identify speakers" per meeting, or the vault-wide "Identify speakers in labelled meetings" backfill) attaches labels + embeddings to an existing `transcript.json` with ids untouched, so hand-made `speakers.json` labels become recognizable voices. A transcript being _created_ with diarization on is additionally cut at changes of voice (`split_segments_at_turns`). **Project speaker roster.** `<vault root>/<PROJECT>/roster.json` (`{schema_version, mode, names}`, `vault::ROSTER_FILE_NAME`, read/written by `commands/roster.rs`) is a _names-only_ roster with a mode: `open` keeps today's free-text box with the sibling names as a datalist, `roster` turns both name controls (`SpeakerNameField` inside `SpeakerTag` and `SelectionSpeakerMenu`) into a strict pick-list over those names. It is edited per project from the recording page's "Project speakers" panel and seeded manually only ("Add names already used in this project"); names are normalized identically in Rust and in `lib/roster.ts`. It holds no embeddings and no segment ids, so the "there is deliberately no `<PROJECT>/speakers.json`" decision stands — voice memory is still the on-demand sibling scan, the service and the MCP server know nothing about the roster, and every project-level walker already skips it because it is a file, not a directory. In `roster` mode the roster additionally bounds the diarization: at submit time `jobs.rs` resolves the meeting's project through `commands/roster.rs::roster_speaker_cap` and sends `max_speakers` = roster size plus a lower `speaker_match_threshold` (from the app-only `speaker_match_threshold_strict` key — `config.rs::strict_speaker_match_threshold`, `0.4` on an untouched install) on the transcribe, re-transcribe and `diarize` submissions of that project's meetings; the service stays roster-agnostic, receiving numbers and never names. Anything that still comes back as a generic `Speaker N` is read by the UI as an unattributed voice ("Unnamed voice", `lib/turns.ts::isGenericSpeakerLabel`) and the service mirrors the same form (`speaker_matching._is_generic`) so a generic label never becomes a named voiceprint for cross-meeting recognition.
