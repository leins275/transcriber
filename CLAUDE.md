...
**Speaker identification is a first-run payload, not a bake.** The installer never ships pyannote/torch; Settings' Speakers row fetches (1) the runtime — every wheel the `diarization` extra adds on top of the baked env, with torch/torchaudio swapped for their `cu126` CUDA builds (~2.7 GB), pinned in `diarization_runtime_packages.py`, which `scripts/gen_diarization_runtime.py` generates from `uv.lock` (`make lint` checks the drift) — into `<app_dir>/runtime/diarization/` (put on `sys.path` by `diarizer.py`), and (2) the three pinned pyannote snapshots into `<app_dir>/models/diarization/` using the operator's Hugging Face token (`hf_token`, typed in `config.rs` alongside `diarize`; both written by `set_diarization_settings`, which restarts the sidecar). Once fetched, the pipeline loads offline (`refs/main` pinned, hub forced offline for the load). NVIDIA GPU only. The project's voice memory stays the on-demand sibling scan — there is deliberately no `<PROJECT>/speakers.json`; the `diarize` job (`commands/speakers.rs`, "Identify speakers" per meeting, or the vault-wide "Identify speakers in labelled meetings" backfill) attaches labels + embeddings to an existing `transcript.json` with ids untouched, so hand-made `speakers.json` labels become recognizable voices. A transcript being *created* with diarization on is additionally cut at changes of voice (`split_segments_at_turns`).
