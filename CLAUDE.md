...
**One serial worker.** All job types (transcribe, summarize, export) share a single serial queue — whisper and the LLM never infer concurrently, and with the default `llm_keep_loaded: false` the GGUF working set is released after each LLM job. Degradation over failure is the pattern throughout: diarization failures and PDF render errors record warnings and still write the primary artifact.
